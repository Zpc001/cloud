...
- **`0014_plugins.sql`**（append-only）：插件市场三张表与枚举放宽：
  ...
  - 放宽 `operations.kind`（+install_plugin/remove_plugin）、`operations.step`（+plugin）、`external_effects.kind`（+plugin_ensure/plugin_delete）；原 CHECK 在 0001 定义，PG 命名为 `表_列_check`，0014 DROP 后以扩展集合重建。
